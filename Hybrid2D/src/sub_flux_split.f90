...
!  Iflag_HLL_HLLC==0 : HLL  ;   ==1 : HLLC       
       ...
       real(sp) :: QL(4),QR(4),UL(4),UR(4),Flux(4),gamma
       real(sp) :: dl,uul,vvl,pl,al, dr,uur,vvr,pr,ar    ! uu velocity
       real(sp) :: p_pvrs,p_star,qql,qqr,Sl,Sr,Fl(4),Fr(4),S_star,tmpl,tmpr,tmp,F_starl(4),F_starr(4)
...
! F=F+ (uL) + F- (uR)	
    ...
    integer:: IFlag_Reconstruction
     real(sp) :: QL(4),QR(4),Flux(4),gamma
     real(sp) :: dl,uul,vvl,pl,al, dr,uur,vvr,pr,ar ,pr1   ! uu velocity
     real(sp) :: tmp0,tmp1,tmp2,tmp3,E1P,E2P,E3P,E1M,E2M,E3M,fp(4),fm(4)
...
!  Code by Leng Yan
     ...
     implicit none
     real(sp) :: QL(4),QR(4),Flux(4),gamma
     real(sp) :: dl,uul,vvl,pl,al, dr,uur,vvr,pr,ar ,Ml,Mr,Mp,Mm  ! uu velocity
     real(sp) :: tmp0,fp(4),fm(4)
...
! F=F+ (uL) + F- (uR)	
     ...
     implicit none
     real(sp) :: QL(4),QR(4),Flux(4),gamma
     real(sp) :: dl,uul,vvl,pl,al, hl,dr,uur,vvr,pr,ar,hr  ! uu velocity
     real(sp) :: a,Ml,Mr,Mp4,Mm4,Pp5,Pm5,M,mp,mm,p,dm,xm
     real(sp) :: fp(4),fm(4)
...
! Roe 型FVS分裂  (Code by LengYan, Revised by Li Xinliang)
  ...
  use const_var ,only :sp,dop
   ...
   real(sp) :: QL(4),QR(4),Flux(4),gamma
   real(sp) :: dl,uul,vvl,pl,al, dr,uur,vvr,pr,ar    ! uu velocity
   real(sp) :: Fl(4),Fr(4),  lamda(4),arr(4), avd,avu,avv,avh,ava, hl,hr,tmpp
   real(sp) ,parameter:: delt=0.1d0   !Harten熵修正的系数 （0至0.125，系数越大粘性越大)
